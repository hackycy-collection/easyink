...
interface DataBinding {
  /**
   * 数据字段路径（全局唯一字段名）
   * 普通元素：绑定标量字段，如 "customerName"、"companyAddress"
   * 表格列：绑定列表字段，如 "orderItemNames"、"orderItemPrices"
   *
   * 绑定统一：文本元素和 Table 列都使用 binding.path，
   * 运行时由数据类型决定行为（标量 vs 数组）。
   * Table 列绑定的字段运行时必须为数组，否则抛出错误。
   */
  path?: string
  /** 表达式（由表达式引擎插件解析） */
  expression?: string
...
5. 运行时通过 `engine.setData()` 一次性传入全量扁平数据

**关键设计决策：**

- **完全扁平化**：没有数组类型的嵌套字段，每个数据源都是全局唯一的 key
  ...
  dataSources: [
    {
      ...
      fields: [
        ...
        },
        {
          title: '订单明细',   // 分组节点
          children: [
            { key: 'itemNames', title: '商品名称' },
            { key: 'itemQuantities', title: '数量' },
            { key: 'itemPrices', title: '单价' },
            { key: 'itemAmounts', title: '金额' },
          ],
        },
      ],
    },
...
所有字段全局扁平，`setData()` 接收一次性全量扁平 map：

```typescript
// 元素绑定示例
{
  binding: { path: 'customerName' }         // 文本元素 → 标量
}
{
  binding: { path: 'companyLogo' }          // 图片元素 → 标量
}
// Table 不持有 binding，每列独立绑定
// columns: [
//   { key: 'name', title: '商品', binding: { path: 'itemNames' } },
//   { key: 'qty',  title: '数量', binding: { path: 'itemQuantities' } },
// ]
...
class DataResolver {
  /**
   * 根据字段 key 从数据中提取值
   * 直接 data[key] 取值，无嵌套路径解析
   *
   * 安全性：禁止访问 __proto__、constructor、prototype（防原型链污染）
   */
  resolve(key: string, data: Record<string, unknown>): unknown

  /**
   * 表格列数据解析：取数组中指定索引的值
   * 运行时校验：如果 data[key] 不是数组，抛出 Error
   */
  resolveColumn(key: string, data: Record<string, unknown>): unknown[]
...
```
1. Table 渲染器收集所有列的 binding.path
2. 对每列调用 DataResolver.resolveColumn(path, data) 获取数组
   - 非数组值 → 抛出 Error
3. 行数 = max(各列数组长度)
4. 短列补 null 至最大行数
5. 按行索引逐行渲染：row[i] = 各列 array[i]
6. 所有列为空数组 → 触发 emptyBehavior
   任一列有数据 → 正常渲染
...
2. 按页面剩余空间计算每页可容纳行数
3. 对每列数据按行区间 slice
4. 短列 null 补齐在 slice 之前（即先补齐再切分）
```
...
interface TableColumn {
  ...
  /**
   * 数据绑定（每列独立绑定一个数据源字段）
   * binding.path 指向一个全局字段 key，运行时该字段的值必须为数组
   * 列间完全隔离，不能跨列引用同行值
   */
  binding?: DataBinding
  /** 格式化器 */
...
| 32 | 数据源命名空间 | 合并单一对象 / 命名空间隔离 / 全局扁平 | **全局扁平（无命名空间）** | 所有字段全局唯一 key，同名后注册覆盖先注册，setData 一次性全量 flat map |
| 33 | 多数据源注册 | 仅初始化 / 动态注册 / 可替换 | **初始化 + 可替换** | 初始化传入 + 支持 unregister/re-register 替换 |
| 34 | 表格数据绑定 | 对象数组 repeat / 每列独立源 / 两者共存 | **每列独立绑定源** | Table 不持有 binding，每列通过 binding.path 独立绑定一个数组字段，行间纯下标对齐 |
...
| 41 | Table 列绑定校验 | 设计时/运行时/不校验 | **仅运行时严格校验** | 注册时不声明维度，设计时无法校验；运行时列绑定非数组直接 throw Error |
...
| 49 | setData 格式 | namespace+flat / 全局flat / 兼容 | **全局 flat map** | 无命名空间，一次性全量传入 |
| 50 | Table 行数 | 最长列 / 要求等长 / 指定主列 | **max(各列长度)** | 短列补 null，行数取最长列 |
| 51 | 标量绑定 Table 列 | 每行重复 / 仅首行 / 禁止 | **禁止（运行时 throw）** | 严格校验，Table 列绑定必须为数组 |
| 52 | Table 分页切行 | 先算行数后切分 / 各列独立切片 | **先算行数再按行切** | 先补齐短列 null，再按页面空间切分行区间 |
