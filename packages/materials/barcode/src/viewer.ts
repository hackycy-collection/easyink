...
export function renderBarcode(node: MaterialNode) {
  ...
  if (!value) {
    return {
      html: trustedViewerHtml(`<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${props.backgroundColor};color:${props.lineColor};font-size:12px;border:1px solid #ddd;">[Barcode]</div>`),
    }
  }
