...
1. Decide whether this is really a material. If it only adds a property, update `propSchemas`; if it adds panels or commands, use Contribution; create a material only when a new Schema node, Designer interaction, and Viewer render path are all needed.
...
5. Register both sides. Built-ins go through `packages/builtin/src/designer.ts` and `packages/builtin/src/viewer.ts`; custom host integrations register through `setupStore` and `viewer.registerMaterial()`.
6. Add `propSchemas` only for simple props-bag fields. Use custom `read` and `commit`, `PropertyPanelOverlay`, or `SelectionType.getPropertySchema()` when a property lives outside `node.props` or changes multiple fields.
7. Add deep editing only when the material has meaningful sub-element selection. Define `MaterialGeometry`, JSON-safe `SelectionType`, behavior middleware, decorations, and `tx.run()` mutations with stable history labels.
8. Add datasource logic at the right layer. Whole-element binding uses `node.binding`; table-like cell binding owns `datasourceDrop` and cell-level `binding` or `staticBinding`.
9. Add i18n keys for visible labels, titles, toolbar text, property labels, reject reasons, and history labels. Prefer `context.t()` and `store.t()` over hardcoded strings.
10. Update AI descriptors when the material is built in or should be generated by MCP/AI flows. Then regenerate or check `packages/mcp-server/config/materials.json`.
11. Test the smallest useful surface: schema defaults, Designer refresh or deep behavior, Viewer render or measure, binding projection, and registration fallout.
