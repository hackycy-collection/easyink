...
function hitTestCell(
  ...
  for (let c = 0; c < topology.columns.length; c++) {
    const colWidth = topology.columns[c]!.ratio * tableNode.width
  ...
  for (let r = 0; r < topology.rows.length; r++) {
    const rowHeight = topology.rows[r]!.height
...
/** Get the next cell in row-major order, wrapping to start at end. */
