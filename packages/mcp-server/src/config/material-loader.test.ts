...
describe('materials config', () => {
  ...
  it('documents aliases as repair hints instead of canonical generation targets', () => {
    ...
    expect(context).toContain('share the element height with the real rows')
    expect(context).toContain('full semantic table box in the designer')
    expect(context).toContain('display-only, share the element height with the real rows, and must not be encoded as extra schema rows')
  })
})
