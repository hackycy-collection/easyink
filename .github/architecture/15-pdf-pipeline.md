...
EasyInk 当前不把 PDF 生成器做成 `core` 的职责。导出能力由独立的 `@easyink/export-runtime`（运行时内核）+ `@easyink/export-adapter-*`（具体格式适配器）承接，`viewer` 只提供页面预览、渲染尺寸、导出/打印调度入口和诊断桥接。运行时与适配器都可被 `viewer` 使用，也可以被 `designer` 的工作台入口直接触发。也就是说，输出能力存在于产品架构中，只是不下沉到 Schema/Core 层。
...
- `@easyink/export-runtime` 提供导出入口、导出任务状态机和导出适配器装载，不绑定任何具体格式。
- `@easyink/export-adapter-dom-pdf` 等独立适配器包提供具体的导出实现并管理各自的第三方依赖。
- `viewer` 与 `designer` 都可以调用导出运行时层。
- 导出适配器负责页面集合、缩略图、字体加载、数据加载、页面样式生成或导出前序列化。
...
interface ExportRuntimeAdapter<TInput = unknown, TResult = Blob | void> {
	id: string
...
// adapter: @easyink/export-adapter-dom-pdf
...
`@easyink/export-runtime` 不依赖 `viewer`。如果宿主要走 `viewer.exportDocument()`，应在宿主侧注册 `ExportAdapter`，把 `ViewerExportContext.container`、`renderedPages`、`onProgress` 和 `onDiagnostic` 桥接给 export runtime。
...
- 浏览器打印：`viewer.print({ adapterId: 'browser' })`，未指定 `adapterId` 时也回退浏览器打印。
- HiPrint 打印：`viewer.print({ adapterId: 'hiprint-adapter', pageSizeMode: 'driver' })`，保留 HTML 逐页打印，继续服务连续纸/驱动介质场景。
- Printer.Host 打印：`viewer.print({ adapterId: 'printer-host-adapter', pageSizeMode: 'fixed' })`，通过 export runtime 生成 PDF 后发送给 Printer.Host。
