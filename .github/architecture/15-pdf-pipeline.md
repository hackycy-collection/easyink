...
EasyInk 当前不把 PDF 生成器做成 `core` 的职责。导出能力由独立的 `@easyink/export-runtime` 承接，`viewer` 只提供页面预览、渲染尺寸、导出/打印调度入口和诊断桥接。这个运行时层可以被 `viewer` 使用，也可以被 `designer` 的工作台入口直接触发。也就是说，输出能力存在于产品架构中，只是不下沉到 Schema/Core 层。
...
- `@easyink/export-runtime` 提供导出入口、导出任务状态机和导出适配器装载。
