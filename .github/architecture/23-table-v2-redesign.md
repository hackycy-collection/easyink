...
| 12 | 移除 `TableDataSchema.source` 字段 | schema, datasource, commands, viewer |
| 13 | repeat-template 行 cell.binding 改为绝对路径 | schema, datasource, viewer |
| 14 | header/footer 行改用 `staticBinding` 替代 `binding` | schema, datasource, commands |
| 15 | 移除 `BindTableSourceCommand` / `ClearTableSourceCommand` | commands |
| 16 | `resolveBindingValue` 移除 `scope` 参数 | datasource, viewer |
| 17 | 新增 `extractCollectionPath` / `resolveFieldFromRecord` 工具函数 | datasource |
...
interface TableCellSchema {
  ...
  // table-data repeat-template 行专用（绝对路径，同行必须共享集合前缀）：
  binding?: BindingRef                  // 仅 table-data repeat-template 行使用
  // table-static 和 table-data header/footer/normal 行共用：
  staticBinding?: BindingRef            // 独立绑定（每 cell 可绑不同 sourceId）
}
```
...
interface TableDataSchema extends TableSchema {
  kind: 'data'
  /** 不再持有 source 字段。集合路径由 repeat-template 行内 cell.binding.fieldPath 的
   *  公共前缀在运行时推导（extractCollectionPath）。 */
...
- table-data 的 header/footer/normal 行也使用 `staticBinding`，共用同一机制
...
- 可通过深度编辑手动输入文本
- 可拖拽数据源绑定（使用 `cell.staticBinding`，绝对路径语义，与 table-static 共用同一机制）
...
- 逐 cell 独立调用 `resolveBindingValue(staticBinding, data)`（绝对路径）
...
| 文档 | 变更内容 |
|------|----------|
| 05-schema-dsl.md | 5.6 节：移除 TableDataSchema.source，repeat-template 行 cell.binding 改为绝对路径，header/footer 行改用 staticBinding，集合路径运行时推导 |
| 06-render-pipeline.md | 6.6.1 节：移除 table.source 读取，改为 extractCollectionPath 推导集合路径，移除 scope 参数 |
| 07-layout-engine.md | 7.3 节：TableMeasureContext 移除 source 引用，集合数据由 extractCollectionPath 推导取出 |
| 08-datasource.md | 8.10-8.12 节：移除 scope 参数，重写 table-data 绑定模型为 cell 级绝对路径 + 同行同集合约束，更新比较表 |
| 10-designer-interaction.md | 10.8 节：移除 table.source 绑定流程，改为 cell 级独立绑定 + 集合前缀校验 |
| 11-element-system.md | 11.3 节：table-data 描述更新为 cell 级绝对路径绑定，属性矩阵更新 |
| 12-command-undo-redo.md | 12.5 节：移除 BindTableSourceCommand/ClearTableSourceCommand，BindStaticCellCommand 扩展到 table-data header/footer |
