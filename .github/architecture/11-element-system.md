...
> **v1 补充边界**：`isContainer` 和 `children` 目前仅为未来受控容器预留。v1 不正式支持通用容器物料，也不承诺任意嵌套后的坐标系、交互和布局行为。**v1 不支持自定义物料**，消费者无需手动注册物料，renderer 和 designer 内置全部物料。自定义物料 API 留到 v2。
...
interface MaterialTypeDefinition {
  ...
  /**
   * 是否触发推移（布局引擎用）
   * true: 该物料高度变化会推移后续未锁定物料
   * false: 该物料不参与推移计算（装饰性物料）
   * 采用白名单模式，默认 false
   */
  causesPush?: boolean
  /**
   * 是否允许旋转
   * true: 设计器中显示旋转手柄，支持自由旋转
   * false: 不允许旋转（如表格类物料）
   * 默认 false
   */
  canRotate?: boolean
