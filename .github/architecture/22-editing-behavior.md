...
> 强制约束：本节描述的三件对象是画布层 selection 与编辑态的**唯一**写入路径。任何在 `*.vue` 组件、`use-*` composable、material extension、editing-session 内部直接调用 `store.selection.{select,add,toggle,clear,selectMultiple}` 的代码都属于违规，PR 必须拒绝。
...
1. 普通单击 click-trigger 物料 → 进入 editing-session，selection 通过 `collapse-to-session-owner` intent 收缩到该元素。
2. Cmd/Ctrl 单击 click-trigger 物料 → 仅画布层 `add` intent，不进入 editing-session。
3. editing-session 活跃且点击的是当前 owner → 事件路由 `editingSession.dispatch`，不动 `SelectionModel`。
4. editing-session 活跃且点击其他元素 → 先 `editingSession.exit()`，再让 controller 重新解释手势。
5. 拖动有位移的 release → click 被 controller 忽略（依据 `gesture.dragMoved`）。
6. Cmd/Ctrl 在未选元素上 pointerdown → controller 写 `add` intent 并设 `selectionAddedViaPrime`；后续 click 直接忽略。Cmd/Ctrl 在已选元素上 click → 写 `toggle`。
7. 右键 → `preserve-for-context-menu` intent；不进 editing，不启动 drag。

### 错误处理边界（落实 22.10 / 21）

- `useElementDrag` 提交事务失败时：执行 `commands.rollbackTransaction()`，记录 `console.error`，**不**把异常抛回浏览器事件循环——一个跨多个节点的 batch 失败不应让标尺/snap-line overlay 卡在半应用态。
- `applySelectionIntent` 的输入是受控的有限枚举；不做 try/catch，错误用类型保证而非运行时。

### 测试护栏

- 单元测试 `packages/designer/src/interactions/canvas-interaction-controller.test.ts` 覆盖审计列出的全部用例（Cmd 多选、drag-then-click、click-trigger 进入、右键保留 etc.）。
