...
- **Editor Core**（详见 §22）：EditorState/Transaction/Step/Plugin/Selection/ViewModel 是 Designer 的内核；旧 Command 类 + DeepEditingDefinition FSM 整体废弃
...
- 属性面板在同一窗口壳层中互斥展示"元素属性"与"页面属性"，并随 `state.selection` 收集各 plugin 的 `PanelContribution` 派生渲染
- 画布中的每个物料必须根据 props 展示近似真实的视觉效果（设计态渲染）
- 设计态渲染（view plugin 的 `render(ctx)` 产出的 VNode）与 Viewer 渲染（`MaterialViewerExtension.render`）是两套独立实现
...
| 10 | [设计器交互层](./10-designer-interaction.md) | 工作台布局、工具组带、窗口系统、结构树、属性壳层、Selection-driven 深度编辑、PanelContribution |
...
| 22 | [Editor Core](./22-editor-core.md) | **新** EditorState/Transaction/Step/Plugin/ViewModel，统一替换旧 Command + FSM 深度编辑模型 |
| 23 | [Table Interaction](./23-table-interaction.md) | **新** 表格交互模型：Notion 风格单 cell 选区 + 浮动工具栏 + 深度面板 + 隐式 cell 类型推断 |
...
- **想了解命令与历史?** -> [12-command-undo-redo](./12-command-undo-redo.md)（**注**：§22 实施后本章将整体替换为 Transaction / History，届时更新）
- **想了解 Editor 内核 / Selection / Plugin / Step?** -> [22-editor-core](./22-editor-core.md)
