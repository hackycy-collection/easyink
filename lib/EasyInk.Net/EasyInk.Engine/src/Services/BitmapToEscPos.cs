...
internal static class BitmapToEscPos
{
    ...
    /// 行主序，MSB=最左。xL/xH = 宽度点数(国产)/字节数(Epson)，yL/yH = 高度点数。
    /// </summary>
    public static List<byte[]> ConvertToBands(Bitmap grayBitmap, int xParamDots, int bandHeightDots)
    {
        ...
        for (int startY = 0; startY < h; startY += bandHeightDots)
        {
            int bandH = Math.Min(bandHeightDots, h - startY);
            bands.Add(BuildBand(grayBitmap, w, xParamDots, bytesPerRow, startY, bandH));
        }
    ...
    private static byte[] BuildBand(Bitmap src, int w, int xParam, int bytesPerRow, int startY, int bandHeight)
    {
        // GS v 0 header: 1D 76 30 m xL xH yL yH
        // xParam: Epson 原厂=bytesPerRow, 国产=w(dots)
        ...
        result[2] = 0x30;
        result[3] = 0;
        result[4] = (byte)(xParam & 0xFF);
        result[5] = (byte)((xParam >> 8) & 0xFF);
