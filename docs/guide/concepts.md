...
设计器编辑 Schema，预览器消费 Schema。模板的导入导出、持久化、历史记录都围绕 Schema 进行。宿主传给设计器的 `schema` 可以是空对象或部分字段，框架会先补齐默认值；设计器内部、自动保存和预览器拿到的始终是完整 `NormalizedDocumentSchema`。
...
物料通过 `capabilities` 声明自身能力：是否可绑定数据、是否可旋转、是否可缩放、是否支持子元素等。
